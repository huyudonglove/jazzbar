  ...
  <div>
      <div>
       <div class="layoutD">
          ...
          <div class="rightD">
              ...
              <el-tab-pane label="组件" v-if="appPower.length" name="first">
                ...
                <el-button style="width:100px;" :disabled="isLook||!appPower[0].isCheck" type="primary" @click="addNode({step:1,text:'场景',type:'场景',data:nodeData1,isLastNode:false,icon:''})">场景</el-button>
                <p class="font20">场景节点</p>
                <el-button style="width:100px;" :disabled="isLook||!appPower[0].isCheck" type="primary" @click="addNode({step:2,text:'载体模型',type:'载体模型',data:nodeData2,isLastNode:false,icon:'gif'})">载体模型</el-button>
                <p class="font20">音乐节点</p>
                <el-button :disabled="isLook||!appPower[0].isCheck" type="primary" @click="addNode({step:3,text:'背景音乐单',type:'背景音乐单',data:nodeData2,isLastNode:true,icon:'img'})">背景音乐单</el-button>
              </el-tab-pane>
              <el-tab-pane label="属性" :disabled="!isPermitClick"  name="second">
                <el-form ref="form" :model="nodeData" label-width="80px">
                  <el-form-item label="场景名称" v-if="nodeData.name!==undefined" required>
                  ...
                  <div style="font-size:20px;font-weight:bold;color:#614a4d;" v-if="type=='场景模型'">载体模型</div>
                  <div style="font-size:20px;font-weight:bold;color:#614a4d;" v-if="type=='背景音乐单'">背景音乐单</div>
                  <div v-if="type=='载体模型'">
                    <carrierModelTable type="box" :disabled="isLook" :isSelectOne="true" :selectArr="nodeData.selectArr?nodeData.selectArr.map(v=>v.id):[]" @change="selectChange"></carrierModelTable>
                  </div>
                  <div v-if="type=='背景音乐单'">
                    <backgroundMusicList type="box" :disabled="isLook" :isSelectOne="true" :selectArr="nodeData.selectArr?nodeData.selectArr.map(v=>v.id):[]" @change="selectChange"></backgroundMusicList>
                  </div>
                </el-form>
              </el-tab-pane>
            </el-tabs>
       ...
       场景列表
        ...
        <sceneTable type="list" @editOrLook="edit" ref="sceneTable"></sceneTable>
    </div>
    <div v-if="showPlay">
...
import carrierModelTable from '../magical/carrierModelTable';
import backgroundMusicList from '../application/backgroundMusicList';
