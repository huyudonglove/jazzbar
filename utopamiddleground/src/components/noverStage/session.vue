  ...
  <div>
      <div>
       <div class="layoutD">
          ...
          <div class="rightD">
              ...
              <el-tab-pane label="组件" v-if="appPower.length" name="first">
                ...
                <el-button style="width:100px;" :disabled="isLook||!appPower[0].isCheck" type="primary" @click="addNode({step:1,text:'场次',type:'场次',data:nodeData1,isLastNode:false,icon:''})">场次</el-button>
                <p class="font20">场景节点</p>
                <el-button style="width:100px;" :disabled="isLook||!appPower[0].isCheck" type="primary" @click="addNode({step:2,text:'场景',type:'场景',data:nodeData2,isLastNode:false,icon:'gif'})">场景</el-button>
                <p class="font20">动画节点</p>
                <el-button  :disabled="isLook||!appPower[0].isCheck" type="primary" @click="addNode({step:3,text:'动画（节目）',type:'动画（节目）',data:nodeData2,isLastNode:true,icon:'img'})">动画（节目）</el-button>
              </el-tab-pane>
              <el-tab-pane label="属性" :disabled="!isPermitClick"  name="second">
                <el-form ref="form" :model="nodeData" label-width="80px">
                  <el-form-item label="场次名称" v-if="nodeData.name!==undefined" required>
                    <el-input :disabled="isLook" v-model="nodeData.name" maxlength="15" @input="change"></el-input>
                  </el-form-item>
                  <el-form-item label="时间" v-if="nodeData.contentTime!==undefined" required>
                    <el-time-picker
                      is-range
                      v-model="nodeData.contentTime"
                  ...
                  <div style="font-size:20px;font-weight:bold;color:#614a4d;" v-if="type=='场景'">场景</div>
                  <div style="font-size:20px;font-weight:bold;color:#614a4d;" v-if="type=='动画（节目）'">动画（节目）</div>
                  <div v-if="type=='场景'">
                    <sceneTable type="box" :disabled="isLook" :isSelectOne="true" :selectArr="nodeData.selectArr?nodeData.selectArr.map(v=>v.id):[]" @change="selectChange"></sceneTable>
                  </div>
                  <div v-if="type=='动画（节目）'">
                    <sceneTable type="box" :disabled="isLook" :isSelectOne="true" :selectArr="nodeData.selectArr?nodeData.selectArr.map(v=>v.id):[]" @change="selectChange"></sceneTable>
                  </div>
       ...
       场次列表
        ...
        <sessionTable type="list" @editOrLook="edit" ref="sessionTable"></sessionTable>
    </div>
    <div v-if="showPlay">
...
export default {
  ...
  methods:{
    judgeTime(time1,time2,type){
      let startTime1 = new Date(`2019/01/01 ${time1[0].slice(11)}`);
      let endTime1 = new Date(`2019/01/01 ${time1[1].slice(11)}`);
      let startTime2 = new Date(`2019/01/01 ${time2[0].slice(11)}`);
      let endTime2 = new Date(`2019/01/01 ${time2[1].slice(11)}`);
      if(type=='包含'){
        return startTime1-startTime2<0||endTime1-endTime2>0;
      }else if(type=='交叉'){
        return (startTime1-startTime2<0&&endTime1-startTime2>0)||(startTime1-endTime2<0&&endTime1-endTime2>0)||(startTime1-startTime2>=0&&endTime1-endTime2<=0&&endTime1-startTime1>0)||(startTime1-startTime2<=0&&endTime1-endTime2>=0);
      }
    },
    boxFull(){
    ...
    playCallback(res){
      console.log(res.detail,1551151);
    ...
    drawLineCallback(res){
      if(res.detail.line){
        console.log(res.detail,15)
        xuminder.delLine(res.detail.line);
      }
    },
      ...
      this.$confirm(`确认保存吗`).then(y=>{
        ...
        if(list[0].id){//是否编辑将第一个编辑后面的新增
          editGroup(list[0]).then(res=>{
            ...
            this.$refs.mp3AnimationTable.getTotal();
            this.loading.close();
          })
