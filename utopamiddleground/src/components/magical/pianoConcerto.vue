  ...
  <div>
      <div>
       <div class="layoutD">
          ...
          <div class="rightD">
              ...
              <el-tab-pane label="组件" v-if="appPower.length" name="first">
                ...
                <el-button style="width:100px;" :disabled="!appPower[0].isCheck||isLook" type="primary" @click="addNode({step:1,text:'钢琴曲',type:'钢琴曲',data:nodeData1,isLastNode:false,icon:''})">钢琴曲</el-button>
                <p class="font20">动画音乐节点</p>
                <el-button style="width:100px;" :disabled="!appPower[0].isCheck||isLook" type="primary" @click="addNode({step:2,text:'动画音乐',type:'动画音乐',data:nodeData2,isLastNode:false,icon:'gif'})">动画音乐</el-button>
                <p class="font20">模型节点</p>
                <el-button style="width:100px;" :disabled="!appPower[0].isCheck||isLook" type="primary" @click="addNode({step:3,text:'icon模型',type:'icon模型',data:nodeData2,isLastNode:true,icon:'img'})">icon模型</el-button>
              </el-tab-pane>
              <el-tab-pane label="属性" :disabled="!isPermitClick"  name="second">
                ...
                <div style="font-size:20px;font-weight:bold;color:#614a4d;" v-if="type=='动画音乐'">动画音乐</div>
                  ...
                  <div v-if="type=='动画音乐'">
                    <cartoonmusicList type="box" :isSelectOne="true" :selectArr="nodeData.selectArr?nodeData.selectArr.map(v=>v.id):[]" @change="selectChange"></cartoonmusicList>
                  </div>
                  <div v-if="type=='icon模型'">
                    <iconModelList type="box" :isSelectOne="true" :selectArr="nodeData.selectArr?nodeData.selectArr.map(v=>v.id):[]" @change="selectChange"></iconModelList>
                  </div>
              </el-tab-pane>
            </el-tabs>
          </div>
       ...
       钢琴曲合集列表
        ...
        <pianoConcertoList type="list" @editOrLook="edit" ref="pianoConcertoList"></pianoConcertoList>
    </div>
   <div v-if="showPlay">
      ...
      this.$confirm(`确认保存吗`).then(y=>{
        ...
        if(list[0].id){//是否编辑将第一个编辑后面的新增
          editGroup(list[0]).then(res=>{
            ...
            this.$refs.pianoConcertoList.getTotal();
            this.loading.close();
          })
