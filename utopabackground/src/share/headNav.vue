...
export default {
 ...
 created(){
  ...
  　　 return (arr=document.cookie.match(reg))?unescape(arr[2]):null;
    ...
    let  token = getCookie(jazzbarName)
    if(token){
    let arr=token.split('.')
    let tokenMsg=arr[1];
...
.nav-common{background:#fff;color:#181C20FF; overflow: hidden; height:60px;border-bottom: 1px solid #eee}
  ...
  .logo div{ background: url("../assets/logo.png") no-repeat 20px  center;text-indent:-999px;width:100%; height: 100%;}
