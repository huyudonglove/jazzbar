       ...
       <div class="layoutD">
          ...
          <div class="rightD">
            <div v-if="isLook" style="width:100%;height:100%;position:absolute;left:0;top:40px;z-index:10;opacity:1;"></div>
              ...
              <el-tab-pane label="组件"  name="first" v-if="scenePower.length">
                <p class="font20">主节点</p>
                <el-button type="primary" style="width:100px;"  @click="addNode({step:1,text:'动画',type:'动画',data:nodeData1,isLastNode:false,icon:'gif'})" :disabled="!scenePower[0].isCheck||isLook">动画</el-button>
                <p class="font20">素材节点</p>
                <el-button style="width:100px;" type="primary" @click="addNode({step:2,text:'Android',type:'android',data:nodeData2,isLastNode:true,icon:''})" :disabled="!scenePower[0].isCheck||isLook">Android</el-button>
                <el-button type="primary" style="width:100px;" @click="addNode({step:2,text:'IOS',type:'ios',data:nodeData2,isLastNode:true,icon:''})" :disabled="!scenePower[0].isCheck||isLook">IOS</el-button>
                <p>
                  <el-button type="primary" style="width:100px;" @click="addNode({step:2,text:'UWP',type:'uwp',data:nodeData2,isLastNode:true,icon:''})" :disabled="!scenePower[0].isCheck||isLook">UWP</el-button>
                  <el-button type="primary" style="width:100px;" @click="addNode({step:2,text:'Windows',type:'windows',data:nodeData2,isLastNode:true,icon:''})" :disabled="!scenePower[0].isCheck||isLook">Windows</el-button>
                </p>
              </el-tab-pane>
  ...
  computed:{
    ...mapState('currentUserPower',['scenePower']),
     nodeColor1(){//节点1必填项判断决定颜色true or false
      return this.nodeData.name&&this.nodeData.materialCategoryId&&this.nodeData.viewType&&this.nodeData.durationTimeStr
    },
