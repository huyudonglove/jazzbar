<template>
    <div>
      <div v-if="type=='list'&&carrierPower.length">
        ...
        <div>
          <el-table :data="tableData" stripe style="width: 100%" @selection-change="handleSelectionChange" border  ref="listTable" :max-height="tableHeight" row-key="id" :cell-class-name="offLine">
            ...
            <el-table-column prop="address" width="210" label="操作" fixed="right">
              <template slot-scope="scope">
                <el-button type="warning" size="mini" :disabled="!carrierPower[2].isCheck" @click="editOrLook(scope.row,true)">查看</el-button>
      ...
      <div v-if="type=='box'">
        <div style="margin-top:10px">
          <el-row class="tac" style="border-bottom:1px solid #eeeeee; padding:15px 0">  
            ...
            <div>
              ...
              <span>
                <huSelect type="animation"  ref="animationBox" @change="changePage" style="display: inline-block;width: 200px"></huSelect>
              </span>
            </div>
          </el-row>
        </div>
        <div>
          <el-table :data="tableData" stripe style="width: 100%" @selection-change="handleSelectionChange" border  ref="boxTable" row-key="id" :cell-class-name="delLine">
            ...
            <el-table-column prop="materialCategoryName" label="类型"></el-table-column>
            <el-table-column prop="placeholderPictureUrl" label="占位图" width="80" align="center">
              <template slot-scope="scope">
                <span style="cursor: pointer;" >
                  <img :src="scope.row.placeUrl?`/static/${scope.row.placeUrl}`:''" alt=""  style="width:50px;height:50px">
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="viewType" label="所属类别"></el-table-column>
          </el-table>
        </div>
        <div>
  ...
  methods:{
    ...
    getBoxTotal(){
      let msg={
        ...
        viewType:this.$refs.animationBox.animationValue,
