        ...
        <el-table :data="tableData" border style="width: 100%" @selection-change="selectAll" :max-height="tableHeight" ref="mangerRef" @sort-change="changeUpadte" >
          ...
          <el-table-column  label="所属模块"  align="center">
            <template slot-scope="scope">
              <span v-if="scope.row.module==108">钢琴管理</span>
            </template>
          </el-table-column>
      ...
      <div>
        <el-dialog title="提示" :visible.sync="dialogVisible" width="40%"  @close="module='';tree=[];"><!--分类栏-->
          ...
          <div style="padding-top: 10px" v-if="module">
            <el-button @click="showLevel(1)" :disabled="!typePower[0].isCheck">新增一级分类</el-button>
          </div>
        ...
        <el-dialog title="提示" :visible.sync="nav" width="30%" ><!--创建输入框提示-->
          ...
          <span slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="addTitle(id)" v-if="this.level==1" :disabled="addDis">新增一级分类</el-button>
            <el-button type="primary" @click="addTitle(id)" v-if="this.level==2" :disabled="addDis">新增二级分类</el-button>
            <el-button type="primary" @click="addTitle(id)" v-if="this.level==3" :disabled="addDis">新增三级分类</el-button>
          </span>
        </el-dialog>
  ...
  import {mapState} from 'vuex'
    export default {
      ...
      computed:{
        ...
        ...mapState('currentUserPower',{'typePower':'deviceTypePower'})
      },
      methods:{
        delectAll(){
          let apple=[];
