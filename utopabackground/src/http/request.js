...
export function applicationUpper(params) {
  return http.get('/api/utopa/ar/background/app/upper', {appId:params})
}
/** 应用管理-下架 w*/
export function applicationLower(params) {
  return http.get('/api/utopa/ar/background/app/lower', {appId:params})
}
/** 应用管理-玩法 w*/
...
/*新增素材 Author:minggui*/
export function addMaterial(msg){
  return new Promise((resolve,reject)=>{
    http.post('/api/utopa/ar/material/add',msg).then(res=>{
      if(res.code){
        Message.error(res.msg);
      }else{
        resolve(res);
...
/*素材下拉框*/
const getSelect=(msg)=>{
  return http.get('/api/utopa/ar/material/module/list',msg)
}
/*新建素材分类*/
const addTitle=(msg)=>{
...
/*应用批量添加*/
const batchAddApp=(msg)=>{
  return loadRequest('/api/utopa/ar/background/app/batchAdd',msg,'增加',http,'post')
  //return http.post('/api/utopa/ar/background/app/batchAdd',msg)
}
/*获取应用素材*/
const pageModuleMaterial=(msg)=>{
  return http.post('/api/utopa/ar/background/app/pageModuleMaterial',msg)
}
export {
  login,
  out,
